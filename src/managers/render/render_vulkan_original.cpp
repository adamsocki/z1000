...
// Forward declarations for IMGUI functions
...
void UpdateMyImgui(Zayn* zaynMem, LevelEditor* editor, Camera* camera, Renderer* renderer, WindowManager* windowManager, InputManager* inputManager) {
    ...
    if (ImGui::Begin("Level Tools", &showLevelTools)) {
        ...
        if (ImGui::Button("Save Level")) {
            // SaveLevel(&zaynMem->levelManager, &zaynMem->gameData, "levels/editor_level.json");
        ...
        if (ImGui::Button("Load Level")) {
            // LoadLevel(&zaynMem->levelManager, &zaynMem->gameData, &zaynMem->entityFactory, 
            //          &zaynMem->meshFactory, &zaynMem->materialFactory, "levels/editor_level.json");
        }
        ...
        if (ImGui::Button("Create Wall")) {
            ...
            // CreateWallEntity(&zaynMem->gameData, &zaynMem->entityFactory, &zaynMem->meshFactory, 
            //                &zaynMem->materialFactory, spawnPos, V3(0,0,0), V3(1,1,1));
        }
        
        ImGui::Separator();
        ImGui::Text("Statistics");
        ImGui::Text("Walls: %d", zaynMem->gameData.walls.count);
        // ImGui::Text("Props: %d", zaynMem->gameData.props.count);
        // ImGui::Text("Lights: %d", zaynMem->gameData.lights.count);
        // ImGui::Text("Triggers: %d", zaynMem->gameData.triggers.count);
    }
    ImGui::End();
